...
function AuthCard({ title, content, isLogin = false }) {
  return (
    <div className="w-full p-10">
      ...
      <p className="text-primary-500 font-bold text-xs md:text-sm text-center max-w-[250px] mx-auto">Herramienta para conocer la salud emocional de los adolescentes</p>
      <p className="text-secondary-400 mt-6 font-bold text-2xl">{title}</p>
      {isLogin ? (
        <p className="text-left mt-2 text-xs text-secondary-100 inline-block">
          ¿No estás registrado? 👉
